...
\begin{titlepage}
    ...
    {\large SaniCer Sanitary Ware Industries Supply Chain Simulator}\\[2cm]
...
\textbf{CeraSim} is a comprehensive discrete-event simulation system designed for \textbf{SaniCer Sanitary Ware Industries}, a Portuguese sanitary ceramics manufacturer based in Aveiro, Portugal. The application models the complete supply chain operations of a commode (toilet) manufacturing facility, from raw material procurement through production stages to customer order fulfillment.

Founded in 1987, SaniCer employs 240 workers and produces three primary product lines: one-piece standard commodes, two-piece economy commodes, and wall-hung premium commodes. The factory operates continuously, processing raw materials (clay, feldspar, silica, and kaolin) through multiple production stages including slip casting, demolding, fettling, glazing, and tunnel kiln firing to produce high-quality sanitary ware.

\subsection{Business Problem and Motivation}

Modern manufacturing operations face complex challenges in supply chain management, capacity planning, and operational optimization. SaniCer management identified several critical business questions:
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{ProductionBatch} - Represents a 50-unit commode batch flowing through the production pipeline
    \begin{itemize}
        \item Encapsulates batch identity, product type, quantity, and quality grades
        \item Tracks stage completion timestamps (casting, demolding, fettling, glazing, firing, finishing)
        \item Maintains quality outcomes and functional test results (leak test, flush test)
    \end{itemize}
...
This design elegantly maps manufacturing reality to software objects—each commode batch is a genuine object with identity and state, machines are resources with capacity constraints, and the factory orchestrates their interactions according to production routing logic.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Optimised} - Capital investment scenario with a second tunnel kiln (+€3.5M CAPEX) and 50\% increased safety stock levels
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Bottleneck:} Tunnel kiln firing (24h/batch, 1 kiln) limits throughput to 1 batch/day (50 units/day)
    \item \textbf{Disruption Impact:} 35-day kaolin strike causes significant production loss affecting customer orders
    \item \textbf{ROI:} Second tunnel kiln increases output by 2×, with 1.2-year simple payback
    \item \textbf{Service Level:} Baseline 35\% fill rate improves to 71\% with second kiln investment
\end{itemize}
...
This diagram models the complete commode manufacturing workflow from raw materials to finished goods. It shows:

\begin{itemize}[leftmargin=*]
    \item \textbf{Decision Points:} Material availability check, functional test pass/fail
    \item \textbf{Sequential Stages:} Slip preparation → pressure casting → demolding → fettling → glazing → tunnel kiln firing → finishing with functional testing
    \item \textbf{Fork/Join:} Quality classification into Pass, Rejects after functional testing
    \item \textbf{Bottleneck Identification:} Tunnel kiln firing (24h cycle) highlighted as the severe constraining resource
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{uml_diagrams/activity_production.png}
    \caption{Activity Diagram - Commode Production Process}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{States:} Created, SlipPrepQueued, SlipPrepInProgress, CastingQueued, CastingInProgress, DemoldingQueued, DemoldingInProgress, FettlingQueued, FettlingInProgress, GlazingQueued, GlazingInProgress, FiringQueued, FiringInProgress, FinishingQueued, FunctionalTesting, Completed
    \item \textbf{Transitions:} Triggered by machine availability and stage completion
    \item \textbf{Testing Transition:} Leak test and flush test determine pass/fail
    \item \textbf{Annotations:} Bottleneck state (FiringQueued - 24h cycle) and cycle time notes
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \texttt{ProductionBatch} hides internal state, exposes properties like \texttt{cycle\_time\_hr}, \texttt{saleable\_units}, \texttt{leak\_test\_pass}, \texttt{flush\_test\_pass}
